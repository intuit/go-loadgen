...
go-loadgen is a log load generator which is meant for testing the logging infrastructure. It is capable of producing structured or unstructured/structure logs with random alphanumeric strings and can also playback any input file at a specified controlled rate. 
## Build
...
Use the help command to find all available commands and flags. 
    ...
    Flags:
          ...
          --duration int                           Duration of the job in seconds (default 300)
          --enable-log-rotation                    by default log rotation is off, use this flag to turn it on. Example --enable-log-rotation will turn log rotation on
      ...
      -h, --help                                   help for loadgen
          --lines-per-second int                   target lines/second (default 1000)
          --output string                          file path of output log file ex: /var/tmp/test.log (default "default.log")
          --log-rotation-max-size-megabytes int   Rotate after N MegaBytes. example 100.  (default 100)
    
    Use "loadgen [command] --help" for more information about a command.
...
> former flag becomes the test termiantion criteria and you cannot simultenously use 2 different termination criteria for the test.
...
go-loadgen can generate a test results log file in the specified path. `--result-log` is the persistent flag which enables that. Use it as shown below. 
